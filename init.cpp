...
{
    // maybe get the retaddr of only one call.
    // eh, this is very inexpensive so it doesn't matter
    DEBUG("LevelShutdown\n");
    og(FASTCALL_CALL);
...
static void hook_impl(std::optional<Target>&& target)
{
#ifdef WIN32
    if (target.has_value() && (MH_CreateHook((void*)target.value(), (void*)(&T::fn), (void**)(&T::og)) == MH_OK))
...
static void ctx_init()
{
    ...
    if (compare_extension_gl.has_value())
        ...
        (void)(LOG("Failed init (compare_extension function nil)\n"), exit(0));
